...
__global__ void kernel_insertimage(CudaImg puvodni, CudaImg vysledny, int2 pozice)
{
    ...
    int l_y = blockDim.y * blockIdx.y + threadIdx.y;
    int l_x = blockDim.x * blockIdx.x + threadIdx.x;
    if (l_y >= vysledny.m_size.y)
        return;
    if (l_x >= vysledny.m_size.x)
        return;
    int pom_y = l_y + pozice.y;
    int pom_x = l_x + pozice.x;
    if (pom_y >= puvodni.m_size.y || pom_y < 0)
        return;
    if (pom_x >= puvodni.m_size.x || pom_x < 0)
        return;
...
__global__ void kernel_resize_zmenseni(CudaImg bigpic, CudaImg smallpic, int zmenseni)
{
    ...
    int l_y = blockDim.y * blockIdx.y + threadIdx.y;
    int l_x = blockDim.x * blockIdx.x + threadIdx.x;
    if (l_y >= smallpic.m_size.y)
        return;
    if (l_x >= smallpic.m_size.x)
        return;
    ...
    if (l_y > 0 && l_x > 0)
    {
        vysledek.x = (bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni - 1).x + bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni).x +
                      ...
                      bigpic.atuchar4(l_y * zmenseni - 1, l_x * zmenseni + 1).x) /
                     9;
        vysledek.y = (bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni - 1).y + bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni).y +
                      ...
                      bigpic.atuchar4(l_y * zmenseni - 1, l_x * zmenseni + 1).y) /
                     9;
        vysledek.z = (bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni - 1).z + bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni).z +
                      ...
                      bigpic.atuchar4(l_y * zmenseni - 1, l_x * zmenseni + 1).z) /
                     9;
        vysledek.w = (bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni - 1).w + bigpic.atuchar4(l_y * zmenseni + 1, l_x * zmenseni).w +
                      ...
                      bigpic.atuchar4(l_y * zmenseni - 1, l_x * zmenseni + 1).w) /
                     9;
    }
...
__global__ void kernel_merge_halves(CudaImg img1, CudaImg img2, CudaImg output)
{
    int l_y = blockDim.y * blockIdx.y + threadIdx.y;
    int l_x = blockDim.x * blockIdx.x + threadIdx.x;

    if (l_y >= output.m_size.y || l_x >= output.m_size.x)
    {
        return;
    }

    int half_width = output.m_size.x / 2;

    if (l_x < half_width)
    {
        output.atuchar4(l_y, l_x) = img1.atuchar4(l_y, l_x);
    }
    else
    {
        output.atuchar4(l_y, l_x) = img2.atuchar4(l_y, l_x - half_width);
    }
}
...
void cu_merge_halves(CudaImg img1, CudaImg img2, CudaImg output)
{
    ...
    kernel_merge_halves<<<l_blocks, l_threads>>>(img1, img2, output);
